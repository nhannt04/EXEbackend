...
public class SpotService {
    ...
    private List<Spot> getAllVirtualSpots() {
        return virtualSpotLoader.getAllVirtualSpots();
    }
    ...
    @org.springframework.cache.annotation.Cacheable(value = "spotById", key = "#id")
    public Spot getSpotById(Long id) {
        if (id == null) return null;
        if (id >= 500000L) {
            return rentalRepository.findById(id - 500000L)
                .map(rental -> Spot.builder()
                    .id(id)
                    ...
                    .images(new ArrayList<>(List.of(SpotImage.builder().id(rental.getId()).imageUrl(rental.getImageUrl()).build())))
                    .build())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa điểm với ID: " + id));
        } else if (id >= 400000L) {
            return entertainmentRepository.findById(id - 400000L)
                .map(ent -> Spot.builder()
                    .id(id)
                    .nameVi(ent.getName())
                    .nameEn(ent.getName())
                    ...
                    .images(new ArrayList<>(List.of(SpotImage.builder().id(ent.getId()).imageUrl(ent.getImageUrl()).build())))
                    .build())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa điểm với ID: " + id));
        } else if (id >= 300000L) {
            return dishRepository.findById(id - 300000L)
                .map(dish -> Spot.builder()
                    .id(id)
                    ...
                    .images(new ArrayList<>(List.of(SpotImage.builder().id(dish.getId()).imageUrl(dish.getImageUrl()).build())))
                    .build())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa điểm với ID: " + id));
        } else if (id >= 200000L) {
            return cafeRepository.findById(id - 200000L)
                .map(cafe -> Spot.builder()
                    .id(id)
                    .nameVi(cafe.getName())
                    .nameEn(cafe.getName())
                    .category("cafe")
                    ...
                    .images(new ArrayList<>(List.of(SpotImage.builder().id(cafe.getId()).imageUrl(cafe.getImageUrl()).build())))
                    .build())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa điểm với ID: " + id));
        } else {
            return stayRepository.findById(id - 100000L)
                .map(stay -> Spot.builder()
                    .id(id)
                    .nameVi(stay.getName())
                    .nameEn(stay.getName())
                    .category("stay")
                    ...
                    .images(new ArrayList<>(List.of(SpotImage.builder().id(stay.getId()).imageUrl(stay.getImageUrl()).build())))
                    .build())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa điểm với ID: " + id));
        }
    }
